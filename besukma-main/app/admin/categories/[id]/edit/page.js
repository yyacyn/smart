...
export default function EditCategory({ params }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.put(`/api/categories/${id}`, formDataToSend, {
        headers: { 
  ...
  if (loading) {
    return (
      <div className="text-slate-500 max-w-3xl">
        <h1 className="text-2xl">Edit <span className="text-slate-800 font-medium">Category</span></h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            >
              ...
              {categories
                .filter(cat => cat.id !== Number(id)) // Exclude self to prevent circular reference
                .map(category => (
